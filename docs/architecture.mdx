...
`vercel.json` builds `contract`, `api`, and `leaderboard-ui`, and serves `leaderboard-ui/dist`. That is the **desk**, not this documentation site. Docs preview and deploy are separate — see [Contributing](/docs/contributing).
